...
class Adapter:
    ...
    def upload_documents(self, documents):
        documents = [document.split('.')[0] + '.json' for document in documents]
        return self._document_handler.upload_documents(documents)

    def get_documents(self, document_id):
        return self._document_handler.get_documents(document_id)
